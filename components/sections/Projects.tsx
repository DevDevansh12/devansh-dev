      ...
      <Container>
        ...
        <div className="grid grid-cols-1 gap-8 md:grid-cols-2">
          {projects.map((p, i) => (
            ...
            >
              {/* Image Banner */}
              <Link href={`/projects/${p.slug}`} className="relative h-60 w-full overflow-hidden bg-slate-100 dark:bg-slate-950 block">
                <Image
                  src={p.image}
                  alt={p.title}
                  fill
              ...
              <div className="flex flex-1 flex-col justify-between p-6 sm:p-7">
                ...
                <div className="mt-6 flex items-center gap-2.5 pt-4 border-t border-slate-100 dark:border-white/10">
                  <Link
                    ...
                    className="inline-flex flex-1 items-center justify-center gap-2 rounded-xl bg-slate-900 px-4 py-2.5 text-xs font-semibold text-white shadow-2xs transition-all duration-150 hover:bg-slate-800 active:scale-[0.98] dark:bg-white dark:text-slate-900 dark:hover:bg-slate-100"
                  >
